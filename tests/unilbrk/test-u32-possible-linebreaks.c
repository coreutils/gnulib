               ...
               int version)
{
...

  {
...

    {
      ...
      my_u32_possible_linebreaks (input, countof (input), "GB18030", p);
...

    {
      ...
      my_u32_possible_linebreaks (input, countof (input), "GB2312", p);
      for (size_t i = 0; i < 61; i++)
        {
          ASSERT (p[i] == (i == 60 ? UC_BREAK_MANDATORY :
                           i == 5
  ...
  /* CR LF handling.  */
  {
    ...
    my_u32_possible_linebreaks (input, countof (input), "UTF-8", p);
